...
struct resolver_entity* resolver_create_new_entity_for_function_call(struct resolver_result* result, struct resolver_process* process, struct resolver_entity* lefy_operand_entity, void* private)
{
  ...
  entity->dtype = lefy_operand_entity->dtype;
...
struct resolver_entity* resolver_get_function_is_scope(struct resolver_result* result, struct resolver_process* resolver, const char* func_name, struct resolver_scope* scope)
{
...
struct resolver_entity* resolver_get_function(struct resolver_result* result, struct resolver_process* resolver, const char* func_name)
{
  ...
  entity = resolver_get_function_is_scope(result, resolver, func_name, scope);
  return entity;
}

struct resolver_entity* resolver_follow_for_name(struct resolver_process* resolver, struct resolver_result* result, const char* name)
{
  ...
  if (entity->type == RESOLVER_ENTITY_TYPE_VARIABLE && datatype_is_struct_or_union(&entity->var_data.dtype) ||
      (entity->type == RESOLVER_ENTITY_TYPE_FUNCTION && datatype_is_struct_or_union(&entity->var_data.dtype)))
  {
...
struct resolver_entity* resolver_follow_identifier(struct resolver_process* resolver, struct resolver_result* result, struct node* node)
{
  struct resolver_entity* entity = resolver_follow_for_name(resolver, result, node->sval);
  if (entity)
  {
...
void resolver_array_bracker_set_flags(struct resolver_entity* bracket_entity, struct datatype* dtype, struct node* bracket_node, int index)
{
...
struct resolver_entity* resolver_follow_array_bracket(struct resolver_process* resolver, struct node* node, struct resolver_result* result)
{
  ...
  struct resolver_entity_rule rule = {};
  resolver_array_bracker_set_flags(array_bracket_entity, &dtype, node, index);
...
{
  return resolver_follow_part_return_entity(resolver, result, node->parenthesis.exp);
}
...
{
  return resolver_follow_part_return_entity(resolver, result, node->unary.operand);
}
...
void resolver_follow_part(struct resolver_process* resolver, struct node* node, struct resolver_result* result)
{
  resolver_follow_part_return_entity(resolver, result, node);
}
