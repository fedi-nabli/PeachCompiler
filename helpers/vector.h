...
// We want at least 20 vector elemnt spaces in the reverse before having
...
struct vector
{
  ...
  // The index will then be incremented
  int pindex;
  int rindex;
  int mindex;
  int count;
...
/**
 * Peeks into the vector of pointers, returning the pointer value itself
 * 
...
void vector_push_at(struct vector* vector, int index, void* elem);
...
/**
 * freads from the file directory into the vector
 */
...
/**
 * Decrements the peek pointer so taht the next peek  
 * will point at the last peeked one
 */
...
/**
 * purges the last save state as if it never happened
 */
...
/**
 * Clones the given vector including all vector data, saves are ignores
 */
